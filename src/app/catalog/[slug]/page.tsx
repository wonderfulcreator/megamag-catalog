...
function marketplaceSearchUrl(market: "ozon" | "wildberries" | "ymarket", q: string) {
  const enc = encodeURIComponent(q);
  if (market === "ozon") return `https://www.ozon.ru/search/?text=${enc}`;
  if (market === "wildberries") return `https://www.wildberries.ru/catalog/0/search.aspx?search=${enc}`;
  return `https://market.yandex.ru/search?text=${enc}`;
}

export default function ProductGroupPage({ params }: { params: { slug: string } }) {
  const group = catalog.groups.find((g) => g.id === params.slug) as ProductGroup | undefined;
          ...
          {group.images.length > 1 && (
            <div className="grid grid-cols-4 gap-2">
              {group.images.slice(0, 8).map((img) => (
                ...
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={`/catalog/${img}`} alt="" className="h-full w-full object-cover" loading="lazy" />
                </div>
              ))}
            </div>
          )}
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-zinc-600">
              <span className="rounded-xl bg-zinc-100 px-3 py-1">{group.type}</span>
              {group.size && (
          ...
          <div className="rounded-3xl border border-zinc-200 p-5">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {group.variants.map((v) => (
                <span key={v.sku} className="rounded-xl bg-zinc-100 px-3 py-1 text-sm">
                  {v.sku}
                </span>
              ))}
            </div>
          ...
          <div className="rounded-3xl border border-zinc-200 p-5">
            ...
            <div className="mt-2 text-sm text-zinc-600">
              Кнопки ведут на поиск по серии/размеру. Если позже дадите точные ссылки на маркетплейсах —
              их можно добавить на уровне SKU в данных.
            </div>
